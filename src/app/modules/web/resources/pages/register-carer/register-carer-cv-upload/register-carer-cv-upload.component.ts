...
import {fileSize, minFileSize, fileType} from '../../../../../../utilities/validators';
...
export class RegisterCarerCvUploadComponent implements OnInit {
    ...
    private minFileSize = 125000; // size in bytes
    ...
    messages = [
        {
            ...
            errors: [
                ...
                {
                    error: 'minFileSize',
                    message: 'Your CV cannot be smaller than 125 kb'
                }
            ]
        },
    ];
    ...
    onFileChange(event) {
        if (event.target.files.length) {
            ...
            if (this.validMimeTypes.indexOf(fileResource.type) !== -1
                && fileResource.size < 1024 * 1024 * this.maxFileSizeMB
                && fileResource.size > 125000) {
            ...
            control.setValidators(
                [Validators.required,
                    fileType(fileResource, this.validMimeTypes),
                    fileSize(fileResource, this.maxFileSizeMB),
                    minFileSize(fileResource, this.minFileSize)
                ]
            );
