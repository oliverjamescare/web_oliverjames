const WEEK_IN_MILLISECONDS = 86400000 * 7;
...
export class DatesService {
    ...
    getStartWeeksDates(): Date[] {
        ...
        for (let i = 1; i < 5; i++) {
            arr.push(new Date(arr[i - 1].getTime() + WEEK_IN_MILLISECONDS));
        }
        return arr;
    }
    ...
    getCalendarLastDayAsString(): string {
        const monday = this.getMonday(new Date());
        const lastDay = new Date(monday.getTime() + WEEK_IN_MILLISECONDS * 34);
        return `${lastDay.getFullYear()}-${lastDay.getMonth()}-${lastDay.getDay()}`;
    }

    isDueJob(jobStart: Date): boolean {
        const now = new Date();
        return (jobStart.getTime() - now.getTime() < 2 * DAY_IN_MILLISECONDS) && this.isUpcomingJob(jobStart);
    }
